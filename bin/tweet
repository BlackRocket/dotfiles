...
loginpage=$(curl -s -b "cookie.txt" -c "cookie.txt" -L --sslv3 -A "$uagent" -d "authenticity_token=$token&username=${TWITTERUSER}&password=${TWITTERPASS}" "https://mobile.twitter.com/session")
